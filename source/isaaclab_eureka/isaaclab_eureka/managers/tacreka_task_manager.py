...
class TacrekaTaskManager:
    """Manages Isaac Lab task training for TacREKA reward functions."""
    ...
    def __init__(
        ...
        self._log_namespace = sanitized_namespace if sanitized_namespace else "tacreka"
    ...
    def _run_seed_in_subprocess(self, reward_func_string: str, seed: int, seed_index: int) -> dict:
        with tempfile.TemporaryDirectory(prefix=f"{self._log_namespace}_seed_{self._idx}_") as temp_dir:
            ...
            if self._video:
                cmd.append("--video")
                cmd.extend(["--video-length", str(self._video_length), "--video-interval", str(self._video_interval)])
    ...
    def _run_training(self, seed: int, framework: Literal["rsl_rl", "rl_games"] = "rsl_rl"):
        ...
        if self._rl_library == "rsl_rl":
            ...
            if self._rl_log_root_dir:
                ...
                log_root_path = os.path.join("logs", "rl_runs", f"rsl_rl_{self._log_namespace}", agent_cfg.experiment_name)
            ...
            if self._rl_log_root_dir:
                ...
                log_root_path = os.path.join("logs", "rl_runs", f"rl_games_{self._log_namespace}", agent_cfg["params"]["config"]["name"])
            log_root_path = os.path.abspath(log_root_path)
            os.makedirs(log_root_path, exist_ok=True)
            print(f"[INFO] Logging experiment in directory: {log_root_path}")
